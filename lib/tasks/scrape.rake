...
task :get_datas => :environment do

	create_run = Run.create(date: Date.today)
	1.upto(17).each do |x|
		url = "http://www.powertrainproducts.net/Engines-s/1.htm?searching=Y&sort=5&cat=1&show=100&page=#{x}"
		...
		page  = agent.get(url)
			...
			page.search('.v65-productDisplay .productnamecolor.colors_productname').each do |x|
				product_url = x['href']
				agent = TorPrivoxy::Agent.new '127.0.0.1', '', {8118 => 9051} do |product_agent|
